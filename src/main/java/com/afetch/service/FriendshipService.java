...
public class FriendshipService {
    ...
    @Transactional
    public UserProfileResponse accept(Long userId, Long friendshipId) {
        Friendship friendship = getPendingForAddressee(friendshipId, userId);
        friendship.setStatus(FriendshipStatus.ACCEPTED);
        friendshipRepository.save(friendship);
        User requester = friendship.getRequester();
        return userService.getProfile(requester.getUsername(), userId);
    }
    ...
    @Transactional(readOnly = true)
    public List<FriendshipResponse> listPendingOutgoing(Long userId) {
        return friendshipRepository.findPendingOutgoingForUser(userId, FriendshipStatus.PENDING).stream()
                .map(this::toResponse)
                .toList();
    }

    @Transactional(readOnly = true)
    public List<UserProfileResponse> listFriendsForViewer(Long targetUserId, Long viewerId) {
        if (!targetUserId.equals(viewerId) && !friendshipRepository.areFriends(targetUserId, viewerId)) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Friends list is private");
        }
        return listFriends(targetUserId);
    }
